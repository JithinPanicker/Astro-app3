...
// ============================================================
// MULTIPAGE PDF UTILITIES
// ============================================================

/**
 * Capture a DOM element as a data URL using html2canvas.
 */
async function captureElement(element, options = { scale: 2 }) {
    const canvas = await html2canvas(element, options);
    return canvas.toDataURL('image/png');
}

/**
 * Clone a template, remove specified selectors, capture, and clean up.
 */
async function captureTemplateSection(templateId, removeSelectors = []) {
    const original = document.getElementById(templateId);
...
async function getWatermarkImage() {
    if (watermarkImageData) return watermarkImageData;
    const watermarkDiv = document.querySelector('.watermark');
    if (watermarkDiv) {
        watermarkImageData = await captureElement(watermarkDiv, { scale: 1, backgroundColor: null });
    } else {
        // fallback: create from logo
        ...
        if (logoImg) {
            ...
            watermarkImageData = canvas.toDataURL('image/png');
        }
    }
    return watermarkImageData;
}

// --- FILL PRESCRIPTION TEMPLATE (used for header capture) ---
function fillPrescriptionTemplate() {
    ...
    // Body is not needed in the template capture; we'll add via jsPDF text.
    return true;
}

// Core prescription PDF generation (returns Blob if returnBlob=true)
async function generatePrescriptionPDFCore(returnBlob = false) {
    const template = getSelectedTemplate();
    ...
    const templateId = template === 'ck' ? 'prescriptionTemplateCK' : 'prescriptionTemplatePratnya';
    const suffix = template === 'ck' ? 'CK' : 'Pratnya';

    // Capture header (letterhead + client details)
    const headerImgData = await captureTemplateSection(templateId, [`#pdfPrescBody${suffix}`, '.letterhead-footer']);
    // Capture footer
    const footerImgData = await captureTemplateSection(templateId, ['.letterhead-header', `#pdfPrescBody${suffix}`, '.watermark']);
    // Watermark
    const watermarkImg = await getWatermarkImage();

    const headerImgProps = pdf.getImageProperties(headerImgData);
    const headerHeightMm = (headerImgProps.height * (pageWidth - 2 * margin)) / headerImgProps.width;
    const footerHeightMm = 20;

    const addPageDecorations = (pagePdf) => {
        pagePdf.addImage(headerImgData, 'PNG', margin, margin, pageWidth - 2 * margin, headerHeightMm);
        if (watermarkImg) {
            pagePdf.addImage(watermarkImg, 'PNG', (pageWidth - 60) / 2, (pageHeight - 60) / 2, 60, 60);
        }
        pagePdf.addImage(footerImgData, 'PNG', margin, pageHeight - margin - footerHeightMm, pageWidth - 2 * margin, footerHeightMm);
    };
    ...
    const textLines = pdf.splitTextToSize(body, pageWidth - 2 * margin);

    let currentY = margin + headerHeightMm + 5;
    const maxY = pageHeight - margin - footerHeightMm - 5;
...
// --- GENERATE PRESCRIPTION PDF (multipage) ---
window.generatePrescriptionPDF = async () => {
    try {
        topToast.fire({ text: 'Generating PDF...' });
        await generatePrescriptionPDFCore(false);
        topToast.fire({ text: 'Downloaded successfully!' });
    } catch (e) {
        console.error(e);
...
// --- SHARE PRESCRIPTION PDF (multipage) ---
window.sharePrescriptionPDF = async () => {
    try {
        const name = document.getElementById('prescName').value || "Client";
        topToast.fire({ text: 'Preparing file for sharing...' });
        const pdfBlob = await generatePrescriptionPDFCore(true);
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
            ...
            Swal.fire({
                title: 'Unsupported Browser',
                text: 'Your device does not support direct file sharing. Please download the PDF and share manually.',
                icon: 'info'
            });
        }
    } catch (e) {
...
// --- GENERATE CLIENT FULL REPORT PDF (multipage with autoTable) ---
window.generatePDF = async () => {
    const template = getSelectedTemplate();
    const name = document.getElementById('name').value || "Client";
    const star = document.getElementById('star').value || "";
    ...
    if (!id) {
        topToast.fire({ text: 'No client loaded', background: '#E0245E' });
        return;
    }
    ...
    try {
        ...
        const templateId = template === 'ck' ? 'pdfTemplateCK' : 'pdfTemplatePratnya';
        const suffix = template === 'ck' ? 'CK' : 'Pratnya';

        // Capture header (letterhead only, no content)
        const headerImgData = await captureTemplateSection(templateId, [`#pdfContent${suffix}`, '.letterhead-footer']);
        const footerImgData = await captureTemplateSection(templateId, ['.letterhead-header', `#pdfContent${suffix}`, '.watermark']);
        const watermarkImg = await getWatermarkImage();
        ...
        let yPos = margin + headerHeightMm + 5;
        pdf.setFontSize(12);
        pdf.setFont('helvetica', 'bold');
        pdf.text('Client Details', margin, yPos);
        yPos += 7;
        pdf.setFontSize(10);
        pdf.setFont('helvetica', 'normal');
        pdf.text(`Name: ${name}`, margin, yPos);
        yPos += 5;
        pdf.text(`Star: ${star}`, margin, yPos);
        yPos += 5;
        pdf.text(`Date of Birth: ${dob}`, margin, yPos);
        yPos += 5;
        pdf.text(`Time of Birth: ${displayTime}`, margin, yPos);
        yPos += 8;
        ...
        if (tableRows.length > 0) {
            pdf.setFontSize(12);
            pdf.setFont('helvetica', 'bold');
            pdf.text('Consultation History', margin, yPos);
            yPos += 5;
            ...
            pdf.autoTable({
                ...
                willDrawPage: (data) => {
                    // On every new page, add header, footer, watermark
                    addPageDecorations(pdf);
                }
            });
        } else {
            pdf.text('No consultation history available.', margin, yPos);
        }
